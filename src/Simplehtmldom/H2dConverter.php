 ...
 *   State
 */
function htmltodocx_insert_html(&$phpword_element, $html_dom_array, &$state = array(), $controls = array()) {
  ...
  // we go:
  htmltodocx_insert_html_recursive($phpword_element, $html_dom_array, $state, $controls);
}
 ...
 */
function htmltodocx_insert_html_recursive(&$phpword_element, $html_dom_array, &$state = array(), $controls = array()) {
  ...
  foreach ($html_dom_array as $element) {
    ...
    $custom = htmltodocx_custom_controls($element, $controls, $state);
    if ($custom) {
        break;
    }
    ...
    switch ($element->tag) {
      ...
      case 'h6':
        ...
        if (in_array($element->tag, $allowed_children)) {
          ...
          htmltodocx_insert_html_recursive($phpword_element, $element->nodes, $state, $controls);
      ...
      case 'sup': // Not working in PHPWord
        ...
        if (isset($element->attr['data-footnote-id'])) {
            $text = htmltodocx_clean_text($element->innertext);
            $footnote = $state['textrun']->addFootnote();
            $footnote->addText($text);
        }
        elseif (in_array($element->tag, $allowed_children)) {
          array_unshift($state['parents'], $element->tag);
          htmltodocx_insert_html_recursive($phpword_element, $element->nodes, $state);
          array_shift($state['parents']);
        }
        else {
            $state['textrun']->addText(htmltodocx_clean_text($element->innertex));
        }
      break;
...
function htmltodocx_custom_controls($element, $controls, $state) {
    $tag = $element->tag;
    $attr = $element->attr;

    foreach ($controls as $control => $callback) {
        $controlSplit = explode('[', $control);
        $controlAttribute = trim($controlSplit[1], '[]');

        if ($tag === $controlSplit[0]) {
            if (! $controlAttribute || ($controlAttribute && isset($attr[$controlAttribute]))) {
                $callback($element, $state['textrun']);
